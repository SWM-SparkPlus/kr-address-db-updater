...
export async function downloadAddressFilesOnly(targetDate: string) {
  downloadPathHandler()
  ...
  const downloadFlag = targetDate.length === 8 ? 'total' : 'daily'
  let url = ''
  let downloadDir = ''
  ...
  logger.info(`[Preparation] Downloads start with ${targetDate}`)
  ...
  logger.info(`[DownloadDailyAddressComplete] Download ${targetDate} completed.`)
}
